...
public class Tetromino {
   ...
   public boolean rotate(GameGrid gameGrid){
      ...
      if(bottomLeftCell.getY() + tileMatrix.length > gridHeight){
         return false;
      }
      ...
      for (int row = 0; row < n; row++) {
            ...
            if(position.getX() < 0 || position.getX() >= gridWidth)
               return false;
               if (gameGrid.isOccupied(position.getX(), position.getY()))
					return false;
			}
		}
      ...
      for (int row = 0; row < n; row++) {       
			for (int col = 0; col < n; col++) {
               ...
               position.setX(bottomLeftCell.getX() + (n-1-row));
               position.setY(bottomLeftCell.getY() + (n-1-col));
         }
      }
